...
type State =
  ...
  | {
      kind: "unavailable";
      reason: "missing" | "malformed" | "expired" | "error";
    }
  | {
      kind: "ready";
      rates: RatesPayload;
      freshness: Exclude<Freshness, "expired">;
    };
    ...
    >
      ...
      <div className="shell">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="label label-rule mb-5 text-gold-antique">
              Market Reference
            </p>
            <h2
              id="rates-heading"
              className="font-display text-section text-ivory"
            >
          ...
          {state.kind === "ready" ? (
            <p
              className="text-[0.68rem] tracking-[0.16em] text-ash uppercase"
              aria-live="polite"
            >
  ...
  return (
    <div className="max-w-2xl" role="status">
      <p className="text-lead text-ivory/72">
        {detail} Rather than show you a number that might be out of date, we
        would rather you asked us directly — we quote against the live market at
        the moment you visit.
      </p>
      <p className="mt-6 text-[0.8rem] leading-relaxed text-ash">
        Metal prices move throughout the trading day. Any figure published here
        is a market reference only; it is never an offer.
      </p>
    </div>
  );
}
    ...
    <>
      ...
      <div className="mt-14">
        ...
        <p className="mt-3 max-w-2xl text-[0.85rem] leading-relaxed text-ivory/72">
          Jewellery is an alloy. These figures are the market value of the pure
          gold inside one gram at each standard fineness — the hallmark number
          stamped on your piece.{" "}
        ...
        <div className="mt-8 overflow-x-auto">
          <table className="w-full min-w-[30rem] border-collapse text-left">
            <caption className="sr-only">
              Market value of the fine gold content of one gram of alloy, by
              hallmark fineness. A market reference only, not an offer.
            </caption>
            <thead>
            ...
            <tbody>
              {GOLD_FINENESS.map(({ karat, fineness, label }) => (
                <tr
                  key={karat}
                  className="border-b border-bronze/20 last:border-0"
                >
                  <td className="py-4 pr-4 font-accent text-[1.05rem] text-ivory">
                    {label}
                  </td>
                  <td className="py-4 pr-4 font-mono text-[0.8rem] text-ash">
                    {fineness}
                  </td>
                  <td className="py-4 text-right font-mono text-[0.95rem] text-gold-high tabular-nums">
                    {formatMoney(
                      finePerGram(gold.perGram, fineness),
                      currency,
                      2,
                    )}
                  </td>
      ...
      <p className="mt-10 max-w-3xl text-[0.75rem] leading-relaxed text-ash">
        ...
        <time dateTime={rates.fetchedAt}>
          {describeAge(rates.fetchedAt, Date.now())}
        </time>
        . Denominated in {currency}. Precious metal prices move throughout the
        trading day, and these figures are published as a market reference only
        — they are not a quotation, an offer, or a guarantee of any amount
        payable.
      </p>
    </>
  );
...
function SpotCard({
  ...
  return (
    <div className="border border-bronze/30 bg-void/40 p-8">
      ...
      <p className="mt-2 text-[0.75rem] tracking-[0.14em] text-ash uppercase">
        {note}
      </p>
      <p className="mt-4 font-mono text-[0.78rem] text-ivory/72 tabular-nums">
        {secondary}
      </p>
    </div>
  );
